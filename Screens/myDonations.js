...
export default class myDonations extends React.Component {
  ...
  getdonations() {
    ...
    var ref = firebase.database().ref("donationpost").orderByChild("email").equalTo(email);
  ...
  async delete(value) {
    ...
    var recentPostsRef = firebase.database().ref('donationpost').orderByChild("uid").equalTo(value);
    recentPostsRef.once('value').then((snapshot) => {
      snapshot.forEach((childSnap) => {
        let userRef = firebase.database().ref(`donationpost/${childSnap.key}`)
        ...
        <View style={{ flexDirection: 'row', marginTop: 10 }}>
          ...
          <View style={{ width: '100%' }}>
            <GradientButton
              ...
              impact
              impactStyle='Light'
              onPressAction={() => this.delete(item.uid)}
            />
          </View>
        </View>
