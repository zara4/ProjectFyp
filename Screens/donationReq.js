...
export default class donationReq extends React.Component {
  ...
  getdonations() {
    var ref = firebase.database().ref("requests").orderByChild("senderid").equalTo(User.uid);
  ...
  navi = async (uid) => {
    ...
    await AsyncStorage.setItem('senderid', uid);
    await AsyncStorage.setItem('chatlist', '1');
        ...
        <View style={{ marginTop: 10 }}>
          ...
          <View style={{ width: '100%' }}>
            <GradientButton
              ...
              impact
              impactStyle='Light'
              onPressAction={() => this.navi(item.uid)}
            />
          </View>
        </View>
